...
//                    ongkir = 48000;
        ...
        btnCekOngkir.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                } else if (etKodePos.getText().length() < 5) {
...
//                    Toast.makeText(getApplicationContext(), "Kolom Alamat wajib diisi", Toast.LENGTH_SHORT).show();
                ...
                } else if (etKodePos.getText().equals("")){
                    etKodePos.setError("Kode Pos Wajib Diisi");
                } else if (etKodePos.getText().length() < 5) {
                    etKodePos.setError("Kode Pos tidak valid");
                } else if (ongkir == 0) {
                    Toast.makeText(OrderActivity.this, "Belum menghitung ongkir!", Toast.LENGTH_SHORT).show();
//                    etKodePos.setError("Isi Kode Pos untuk menghitung ongkir");
                }else {
    ...
    public void buatTransaksi() {
        ...
        APIRequestData apiRequestData = RetroServer.koneksiRetrofit().create(APIRequestData.class);
        Call<ResponseTransaksi> transaksi = apiRequestData.createTransaksi(id, penerima, alamat, no_telp, ongkir);
         ...
         callOngkir.enqueue(new Callback<ResponseOngkir>() {
             ...
             public void onResponse(Call<ResponseOngkir> call, Response<ResponseOngkir> response) {
                 if (response.body().getStatus().equals("SUCCESS")) {
                     ...
                     vOrder.setVisibility(View.GONE);
                     pbOrder.setVisibility(View.GONE);
                 } else if (response.body().getStatus().equals("NOT CONNECTED")) {
                     vOrder.setVisibility(View.GONE);
                     pbOrder.setVisibility(View.GONE);
                     Toast.makeText(getApplicationContext(), "Terjadi kesalahan saat menghubungi server", Toast.LENGTH_SHORT).show();
                 }
             }
